...
public class EmployeeDao {
	...
	public Employee selectOne(Connection conn, String empId) throws EmployeeException {
		...
		try {			
			...
			if(emp == null)
				throw new EmployeeException(empId + "사번에 대한 직원정보가 존재하지 않습니다.");
	...
	public HashMap<String, Employee> selectJobMap(
			Connection conn, String jobId) throws EmployeeException {
		HashMap<String, Employee> empMap = 
				new HashMap<String, Employee>();		
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		try {			
			...
			while(rset.next()) {
				...
				empMap.put(jobId, emp);
			}
			if(empMap.size() == 0)
				throw new EmployeeException(jobId + "사원 정보가 존재하지 않습니다.");
	...
	public HashMap<String, Employee> selectDeptMap(
			Connection conn, String deptId) throws EmployeeException{
		HashMap<String, Employee> empMap = 
				new HashMap<String, Employee>();		
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		try {			
			...
			while(rset.next()) {
				...
				empMap.put(deptId, emp);
			}
			if(empMap.size() == 0)
				throw new EmployeeException(deptId + "사원 정보가 존재하지 않습니다.");
	...
	public int updatePhone(
			Connection conn, Employee emp) throws EmployeeException {
		int result = 0;		
		PreparedStatement pstmt = null;
		...
		try {			
			...
			if(result <= 0) {
				rollback(conn);
				throw new EmployeeException("직원정보 수정 실패");}
	...
	public int delete(Connection conn, String empId) throws EmployeeException {
		...
		try {			
			...
			if(result <= 0) {
				rollback(conn);
				throw new EmployeeException("직원정보 삭제 실패");}
